...
function cbSRSuccess(searchData) {
    ...
    if(searchData.getCount() > 0) {
    	...
    	odkTables.openTableToListView(
				...
				'_id = ?',
				[rowId],
				'config/tables/pediatria/html/pediatria_discharge_old.html');
    } else {
...
function cbSRFailure(error) {
    console.log('pediatria_hospcheck: cbSRFailure failed with error: ' + error);
}
...
function cbFailure(error) {
    console.log('pediatria_discharge: failed with error: ' + error);
}

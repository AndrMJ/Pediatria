...
function cbSRSuccess(searchData) {
    ...
    if(searchData.getCount() > 0) {
    	...
    	odkTables.openTableToListView(
				...
				'_id = ?',
				[rowId],
				'config/tables/pediatria/html/pediatria_laterounds_old.html');
    } else {
...
function cbSRFailure(error) {
    console.log('pediatria_laterounds: cbSRFailure failed with error: ' + error);
}
...
function render() {
    ...
    for (var i = 0; i < pediatria.getCount(); i++) {
        ...
        var roundsdateEntered = pediatria.getData(i, 'roundsdatelate');
        var td = new Date();
        var m = td.getMonth();
        var d = td.getDate();
...
function cbFailure(error) {
    console.log('pediatria_laterounds: failed with error: ' + error);
}

...
function cbSRSuccess(searchData) {
    ...
    if(searchData.getCount() > 0) {
    	...
    	odkTables.openTableToListView(
				...
				'_id = ?',
				[rowId],
				'config/tables/pediatria/html/pediatria_initrounds_old.html');
    } else {
...
function cbSRFailure(error) {
    console.log('pediatria_initrounds: cbSRFailure failed with error: ' + error);
}
...
function render() {
    ...
    // create button that adds patients to the system - launches initroundsQuick form
    ...
    for (var i = 0; i < pediatria.getCount(); i++) {
        ...
        // make list entry
        // Only show patients not yet seen
...
function cbFailure(error) {
    console.log('pediatria_initrounds: failed with error: ' + error);
}
